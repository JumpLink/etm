...
function linkPkg (dir, pkg, cb) {
  var dstPath = path.join(dir, 'views', 'themes', pkg.name)
  var srcPath = path.relative(
    path.join(dstPath, 'views', 'themes'),
    path.join(dir, 'views', 'themes', pkg.uid)
  )

  // path.dirname('@hello/world') #=> 'hello'
  // path.dirname('world') #=> '.'
  async.series([
    mkdirp.bind(null, path.join(dir, 'views', 'themes', path.dirname(pkg.name))),
...
function recurse (dir, tmpDest, pkg, cb) {
  ...
  async.series([
    ...
    mkdirp.bind(null, path.join(tmpDest, 'views', 'themes')),
